...
import pandas as pd
import numpy as np
import random, math, datetime, io
...
def generate_synthetic_data(n=10000, seed=42):
    ...
    for _ in range(n):
        rand_ts = start_dt + datetime.timedelta(seconds=random.uniform(0, total_seconds))
        month = rand_ts.month
        ...
        order_profit = int(random.uniform(*base_profit_range))
...
def api_data():
    """
    Returns the full dataset. If you'd like to limit the initial payload,
    add a `limit` query param, e.g. /api/data?limit=1000
    """
    limit = request.args.get("limit", type=int)
    if limit and limit > 0:
        return jsonify(DATA_DF.head(limit).to_dict(orient="records"))
...
def api_filter():
    ...
    # monthly aggregates (average delay by month)
    if total > 0:
        monthly = filtered.copy()
        monthly["month"] = pd.to_datetime(monthly["date_orders"]).dt.to_period("M").astype(str)
        monthly_group = monthly.groupby("month")["shipping_delay_days"].mean().sort_index()
        labels = list(monthly_group.index)
        avg_delays = list(monthly_group.fillna(0).values)
    else:
        labels, avg_delays = [], []

    # country risk percentages
    if total > 0:
        country_group = filtered.groupby("customer_country").agg(
            total=('late_delivery_risk', 'size'),
            late=('late_delivery_risk', 'sum')
        )
        countries = list(country_group.index)
        risk_percent = list((country_group['late'] / country_group['total'] * 100).fillna(0).values)
    else:
        countries, risk_percent = [], []
    ...
    resp = {
        ...
        "kpis": {
            "total_orders": total,
            "late_delivery_percent": late_pct,
    ...
    for _ in range(num_sim):
        ...
        while t < horizon:
            ...
            if distribution == "weibull":
                inter_t = scale * ((-ln(1 - u)) ** (1.0 / shape))
            else:
            ...
            if t <= horizon:
                disruptions += 1
                total_cost += avg_cost * (0.5 + rng() * 1.5)
...
if __name__ == "__main__":
    # Bind to PORT for Render/local; no debug in production
